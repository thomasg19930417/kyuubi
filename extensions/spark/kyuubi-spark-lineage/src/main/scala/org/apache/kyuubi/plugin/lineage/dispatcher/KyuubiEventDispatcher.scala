...
import org.apache.kyuubi.plugin.lineage.{Lineage, LineageDDL, LineageDispatcher}

class KyuubiEventDispatcher extends LineageDispatcher {

  override def send(
      qe: QueryExecution,
      lineageDDl: Option[LineageDDL],
      lineage: Option[Lineage]): Unit = {
    val event =
      OperationLineageKyuubiEvent(qe.id, System.currentTimeMillis(), lineageDDl, lineage, None)
    EventBus.post(event)
  }

  override def onFailure(qe: QueryExecution, exception: Exception): Unit = {
    val event =
      OperationLineageKyuubiEvent(qe.id, System.currentTimeMillis(), None, None, Some(exception))
    EventBus.post(event)
  }
...
case class OperationLineageKyuubiEvent(
    executionId: Long,
    eventTime: Long,
    ddlOperator: Option[LineageDDL] = None,
